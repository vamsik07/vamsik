...
const tempDataReceieved = (state, action) => {
  const data = action.data.data.map((point)=>{
    return  {
      name: point.timestamp, pv: point.metric
    }
  })
...
const handlers = {
  ...
  [actions.TEMP_DATA_RECEIVED]: tempDataReceieved
};
